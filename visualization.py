...
if __name__ == '__main__':
    ...
    samples = CounterValue(labels.numpy().reshape(-1))
    ...
    # 睡眠阶段分期图
    eeg_data, labels = getEEGData(h5file, [files[9]])
    print(eeg_data.shape)
    model = MSNN()
    model.load_state_dict(torch.load(modelPath + model_files[0]))
    standardscaler = pickle.load(open(standardscalerPath + standardscaler_files[0], 'rb'))
    PlotSleepPicture(model, standardscaler, eeg_data, labels, sleeppicturePath, "7_")

    # 特征过滤器激活图
    files_index = 35
    model_index = 1
    flag = 2
    eeg_data, labels = getEEGData(h5file, [files[files_index]])

    model = MSNN()
    model.load_state_dict(torch.load(modelPath + model_files[model_index]))
    standardscaler = pickle.load(open(standardscalerPath + standardscaler_files[model_index], 'rb'))
    ...
    for stage in range(5):
        ...
        standardscaler = pickle.load(open(standardscalerPath + standardscaler_files[1], 'rb'))
    ...
    sns.heatmap(data=out, square=True, cbar=False, cmap="GnBu", fmt='', annot=pred_y_list, annot_kws={'size':6, 'color':'black'})
    plt.yticks([])
    plt.xticks([])
